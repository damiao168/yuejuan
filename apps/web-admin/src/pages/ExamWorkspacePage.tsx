...
import { Alert, Button, Progress, Space, Statistic, Tabs } from "antd";
...
const sections = [
  ["overview", "概览"], ["students", "学生"], ["paper", "试卷"], ["questions", "题目与评分标准"],
  ["template", "答题卡模板"], ["settings", "开考准备"], ["capture", "采集"], ["processing", "识别处理"],
  ["grading", "阅卷"], ["quality", "质检仲裁"], ["scores", "成绩"], ["appeals", "申诉"], ["reports", "报告"]
] as const;
...
const sectionDestinations: Record<string, { path: string; action: string; description: string }> = {
  ...
  questions: { path: "/papers", action: "配置题目与评分标准", description: "建立题目、答案和主观题评分规则。" },
  template: { path: "/papers", action: "配置答题卡模板", description: "在试卷底图上框选每道题的答题区域并锁定版本。" },
  capture: { path: "/capture", action: "进入答卷采集", description: "导入答卷并执行页面质量检查。" },
  processing: { path: "/capture", action: "查看处理进度", description: "查看答卷的图像质量检查、文字识别与答题区域切分进度。" },
  grading: { path: "/grading", action: "进入阅卷", description: "领取或继续处理本场考试的阅卷任务。" },
  quality: { path: "/arbitration", action: "处理双评差异与仲裁", description: "查看双评差异和待仲裁事项。" },
  ...
  settings: { path: "/exams", action: "开考准备检查", description: "查看未完成的准备项，全部通过后确认并开始采集。" }
};
    ...
    <div className="exam-workspace-page">
      <header className="exam-context-header">
        ...
        <div className="exam-context-title">
          <span title={data.exam.subject}>{examSubjectLabel(data.exam.subject)} · {data.gradeLabel}</span>
          <h1>{data.exam.name}</h1>
          <Space size="small"><StatusTag tone={examStatusTone(data.exam.status)}>{examStatusLabels[data.exam.status] ?? "未知状态"}</StatusTag><span>负责人：{data.exam.created_by === currentUser.id ? currentUser.name : "本校管理员"}</span></Space>
        </div>
        <div className="exam-context-progress"><span>总体进度</span><Progress percent={progressByStatus[data.exam.status] ?? 0} size="small" format={() => examStatusLabels[data.exam.status] ?? ""} /></div>
      ...
      <nav className="exam-section-nav" aria-label="考试工作区导航">
        <Tabs type="line" activeKey={visibleSections.some(([key]) => key === section) ? section : "overview"} items={visibleSections.map(([value, label]) => ({ key: value, label }))} onChange={(value) => onNavigate(`/exams/${encodeURIComponent(examId)}/${value}`)} />
      </nav>
      ...
      {section === "overview" ? (
        <main className="exam-overview">
          <section className="exam-facts">
            <Statistic title="参与班级" value={data.exam.class_ids.length} suffix="个班级" />
          ...
          <div className="exam-overview-grid">
            <section className="workspace-section readiness-section">
              <div className="section-head"><div><h2>当前下一步</h2></div></div>
              {readyIssues.length ? <div className="readiness-list">{readyIssues.map((issue) => <button type="button" key={issue.code} onClick={() => onNavigate(`/exams/${encodeURIComponent(examId)}/${issue.section}`)}><CircleAlert size={17} /><span>{issue.label}：{issue.message}</span><ArrowRight size={16} /></button>)}</div> : <div className="ready-line"><CheckCircle2 size={18} /><span>{data.exam.status === "published" ? "成绩已经发布，可进入成绩页查看发布结果" : data.exam.status === "archived" ? "考试已经归档，成绩保持只读" : data.readiness?.confirmed ? "开考准备已经确认" : "基础配置已通过，可以进行开考确认"}</span></div>}
            </section>
            <section className="workspace-section risk-section">
              <div className="section-head"><div><h2>需要关注</h2><p>待人工确认的答卷</p></div><strong>{data.reviewRequiredCount}</strong></div>
        ...
        <main className="workspace-section workspace-section-landing">
          <div><span className="dashboard-kicker">{sections.find(([key]) => key === section)?.[1] ?? "考试工作区"}</span><h2>{destination?.action ?? "返回考试概览"}</h2><p>{destination?.description ?? "该环节暂未开放。"}</p></div>
