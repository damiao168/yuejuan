...
- 学生条码 v2 已完成考试 roster 约束、不可变 print batch/sheet/page 台账、签发幂等、未签发/已作废 serial 拒绝、跨文件重复页冲突与解除重算、带原因和替代链的作废/重印 API，以及由不可变台账生成的确定性 A4 PDF。Web 已支持按班级签发并下载、刷新恢复历史批次、排除缺考和已有有效答题卡学生，并在批次开始扫描后禁止重复下载。三页插页/缺页/重复学生联合 PostgreSQL E2E 已通过：插入页不会挤乱受控页码，缺第 2 页时第 3 页仍保持第 3 页；尚缺 30 份实体回扫报告。
- OMR 校准已上移到不可变模板 profile 作用域：全模板合计 100 份，按题目和高置信 / 低置信 / 空白 / 模糊结果分层抽样；未标注前服务端隐藏机器判定，审批只放行经人工验证零错误的高置信分支。批准证据可随内容哈希完全相同的模板克隆，并保留来源会话；版式、profile 或对照页变化后自动失效。旧 question 作用域记录仅兼容读取，不会被静默扩权。
- 采集自救已完成：失败 capture file 可在原批次恢复真实 Worker task；失败源文件不再永久占用 hash，修复后的同文件可重传，活跃/已完成文件仍防重复；质量误判页可由有权限人员填写原因后放行，保留原机器状态、问题、操作者和时间，并自动恢复版面对齐任务。
- 旧 submission 直传与直接切题入口已通过标准 `Deprecation` / `Sunset` / successor link 响应头及兼容页面警示明确标记废弃；生产采集入口统一指向 capture batch。
