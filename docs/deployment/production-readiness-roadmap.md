...
> 规划版本说明（2026-07-12）：本文档中早期 STORY-053～057 的编号与后续 V1.0 正式产品交付总控任务存在历史错位。后续执行以 `docs/stories/README.md` 和 V1.0 总控顺序为准：STORY-055 为采集到自动切题，STORY-056 为客观题评分引擎和专业人工阅卷工作台，STORY-057 为真实主观题 AI、证据校验、答案分组和模型门禁，STORY-058 为阅卷质量中心，STORY-059 为发布/学生端/申诉/报告，STORY-060 为 Windows 离线采集与用户/运维治理，STORY-061 为正式发布门禁。旧表保留为历史缺口分析，不能再作为 Story 编号依据。

> 执行状态（2026-07-15）：STORY-056 已完成实现复审并批准；STORY-057～059 保持冻结，未纳入本次实现或提交。
