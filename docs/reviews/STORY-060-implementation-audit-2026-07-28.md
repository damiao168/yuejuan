...
> 2026-07-29 最终更新：本审计指出的 STORY-060B、060C 缺口已收口，060D 软件联合验收也已通过。模板 profile 级校准、无偏分层抽样、受控克隆继承、采集自救、质量人工放行、五组 PostgreSQL 联合场景、真实浏览器流程和全量工程门禁均有通过证据。当前状态为 `Software Accepted / Physical Validation Pending`；30 份真实打印回扫仍是外部验收硬门禁。详见 [`STORY-060D 软件联合验收记录`](STORY-060-software-acceptance-2026-07-29.md)。下文保留 7 月 28 日审计原貌，作为变更前基线。
...
2026-07-29 已补齐 Web 操作闭环：锁定模板页通过 `exam:manage` 专用上下文读取应考学生和最近 20 个签发批次，按班级签发时自动排除缺考与已有有效答题卡学生；签发成功自动下载 PDF，刷新后仍可恢复尚未扫描的历史批次。真实浏览器已完成 4 人、8 页 PDF 签发下载回归，测试数据随后精确清理。
