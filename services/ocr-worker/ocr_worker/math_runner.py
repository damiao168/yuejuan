...
class MathVerificationClient:
    ...
    def normalize(self, latex: str) -> dict[str, Any]:
        body = json.dumps({"latex": latex, "domain": {"name": "real"}}).encode()
        req = request.Request(
            f"{self.base_url}/internal/math/normalize",
            data=body,
...
def _build_artifact(task: dict[str, Any], routed: Any, verifier: MathVerificationClient) -> dict[str, Any]:
    ...
    if routed.kind is RegionKind.TEXT and routed.text_blocks:
    ...
    step_formula_ids = ["formula-1"] if formulas else []
    ...
    return {
        ...
        "engine_version": "math-runtime-v1",
        "blocks": blocks,
        "formulas": formulas,
        "relations": [],
        "solution_graph": {
            ...
            "answer_segment_id": segment_id,
            "builder_version": "math-runtime-v1",
            "formula_model_version": formulas[0]["recognition_version"] if formulas else "text-only",
...
def _box(value: object) -> dict[str, float]:
