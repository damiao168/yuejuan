...
func TestPostgresExamWritesCheckParentTenant(t *testing.T) {
	...
	for _, want := range []string{
		...
		"FROM school_class WHERE tenant_id = $1 AND id::text = $2 AND deleted_at IS NULL",
	} {
		if !strings.Contains(sqlText, want) {
			t.Fatalf("Postgres exam writes must check parent tenant with %q", want)
		}
	}
}
